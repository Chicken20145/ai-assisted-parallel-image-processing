...
__global__ void equalize_kernel(
    ...
    const unsigned int* cdf, unsigned int cdf_min) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index >= pixels) return;
...
ProcessingResult process_cuda(
    ...
    try {
        ...
        if (algorithm == Algorithm::GaussianBlur) weights = make_gaussian_kernel(params.kernel_size, params.sigma);
        ...
        if (algorithm == Algorithm::HistogramEqualization) check_cuda(cudaMalloc(&device_histogram, 256 * sizeof(unsigned int)), "Cấp phát histogram");
        ...
        if (algorithm == Algorithm::GaussianBlur) {
            gaussian_kernel<<<grid2d, block2d>>>(device_input, device_output, input.width, input.height, input.channels, device_weights, params.kernel_size);
        } else {
            grayscale_kernel<<<blocks, 256>>>(device_input, device_gray, pixels, input.channels);
            if (algorithm == Algorithm::Sobel) {
                sobel_kernel<<<grid2d, block2d>>>(device_gray, device_output, input.width, input.height, params.threshold);
            } else {
                ...
                std::vector<unsigned int> histogram(256), cdf(256);
                check_cuda(cudaMemcpy(histogram.data(), device_histogram, 256 * sizeof(unsigned int), cudaMemcpyDeviceToHost), "Đọc histogram");
                cdf[0] = histogram[0];
                for (int i = 1; i < 256; ++i) cdf[i] = cdf[i - 1] + histogram[i];
                unsigned int cdf_min = 0;
                for (unsigned int value : cdf) if (value != 0) { cdf_min = value; break; }
                check_cuda(cudaMemcpy(device_histogram, cdf.data(), 256 * sizeof(unsigned int), cudaMemcpyHostToDevice), "Chép CDF");
                equalize_kernel<<<blocks, 256>>>(device_gray, device_output, pixels, device_histogram, cdf_min);
            }
        }
